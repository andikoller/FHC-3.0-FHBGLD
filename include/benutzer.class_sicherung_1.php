...
class benutzer extends person
{
	...
	public function save($new=null, $saveperson=true)
	{
		...
		if($new) //Wenn new true ist dann ein INSERT absetzen ansonsten ein UPDATE
		{
			...
			//Andreas Koller: Benutzerpasswort generieren
			$char_control = NULL;
			
			while ((preg_match('/[[:digit:]]/', $char_control) != 1) || (preg_match('/[[:lower:]]/', $char_control) != 1) || (preg_match('/[[:upper:]]/', $char_control) != 1) || (preg_match('/[[:punct:]]/', $char_control) != 1) || ($char_control < 8))
			{
				//Passwort-Funktion
				//Folgende Zeichen werden nicht verwendet:  j, i, l, I, o, O, Q, 1, O
				$length = 8;
				
				$char_control  = ""; 
				$chars_for_pw  = "ABCDEFGHJKLMNPRSTUVWXYZ"; 
			...
			$benutzerpasswort = $char_control;
			
			$qry = 'INSERT INTO public.tbl_benutzer (uid, aktiv, alias, person_id, insertamum, insertvon, updateamum, updatevon, ext_id, aktivierungscode, passwd) VALUES('.
				   ...
				   $this->db_add_param($benutzerpasswort).");";
		}
		else
		{			
