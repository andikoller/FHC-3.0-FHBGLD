...
class moodle24_user extends basis_db
{
	...
	public function createUser($uid)
	{
		if($uid=='_DummyLektor')
			return true;
		//Andreas Koller: AND vorname IS NOT NULL hinzugefügt, da dadurch Geräte aus dem Labor Studiengang nicht im Resultset sind.
		$qry = "SELECT uid, vorname, nachname FROM campus.vw_benutzer WHERE uid=".$this->db_add_param($uid)."AND vorname IS NOT NULL";
		if($this->db_query($qry))
		{
