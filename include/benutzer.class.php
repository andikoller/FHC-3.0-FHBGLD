...
class benutzer extends person
{
	...
	public function save($new=null, $saveperson=true)
	{
		...
		if($new) //Wenn new true ist dann ein INSERT absetzen ansonsten ein UPDATE
		{
			...
			while ((preg_match('/[[:digit:]]/', $char_control) != 1) || (preg_match('/[[:lower:]]/', $char_control) != 1) || (preg_match('/[[:upper:]]/', $char_control) != 1) || (preg_match('/[[:punct:]]/', $char_control) != 1) || (strlen($char_control) < 8))
			{
				//Passwort-Funktion
				$length = 8;
