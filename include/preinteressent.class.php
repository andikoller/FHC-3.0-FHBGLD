...
class preinteressent extends basis_db
{
	...
	//Andreas Koller - Um ZGV Daten ergaenzt:
	public $zgv_code;			// integer
	public $zgvort;				// varchar(64)
	public $zgvdatum;			// date
	public $zgvmas_code;		// integer
	public $zgvmaort;			// varchar(64)
	public $zgvmadatum;			// date
	public $zgvdoktor_code;		// integer
	public $zgvdoktorort;		// varchar(64)
	public $zgvdoktordatum;		// date
	...
	public function delete($preinteressent_id)
	{
		...
		if($this->db_query($qry))
		{
			if($row = $this->db_fetch_object())
			{
			//Andreas Koller - undo um ZGV-Daten ergaenzt:
				$undo.=" INSERT INTO public.tbl_preinteressent(preinteressent_id, person_id, studiensemester_kurzbz, 
						...
						insertamum, insertvon, updateamum, updatevon, maturajahr, infozusendung, kontaktmedium_kurzbz,
						zgv_code, zgvort, zgvdatum, zgvmas_code, zgvmaort, zgvmadatum, zgvdoktor_code, zgvdoktorort, zgvdoktordatum) VALUES (".
						...
						//Andreas Koller - um ZGV-Daten ergaenzt:
						$this->addslashes($row->zgv_code).', '.
						$this->addslashes($row->zgvort).', '.
						$this->addslashes($row->zgvdatum).', '.
						$this->addslashes($row->zgvmas_code).', '.
						$this->addslashes($row->zgvmaort).', '.
						$this->addslashes($row->zgvmadatum).', '.
						$this->addslashes($row->zgvdoktor_code).', '.
						$this->addslashes($row->zgvdoktorort).', '.
						$this->addslashes($row->zgvdoktordatum).');';
	...
	public function save($new=null)
	{
		...
		if($new)
		{
			...
			$qry = "BEGIN;INSERT INTO public.tbl_preinteressent (studiensemester_kurzbz, 
					...
					maturajahr, infozusendung, person_id, updateamum, updatevon, insertamum, insertvon, kontaktmedium_kurzbz,
					zgv_code, zgvort, zgvdatum, zgvmas_code, zgvmaort, zgvmadatum, zgvdoktor_code, zgvdoktorort, zgvdoktordatum) VALUES (".
		...
		else 
		{
			...
			$qry= "UPDATE public.tbl_preinteressent SET".
				  ...
				  " zgvort=".$this->addslashes($this->zgv_code).",".
				  " zgvdatum=".$this->addslashes($this->zgv_code).",".
				  " zgvmas_code=".$this->addslashes($this->zgvdatum).",".
				  " zgvmaort=".$this->addslashes($this->zgvmaort).",".
				  " zgvmadatum=".$this->addslashes($this->zgvmadatum).",".
				  " zgvdoktor_code=".$this->addslashes($this->zgvdoktor_code).",".
				  " zgvdoktorort=".$this->addslashes($this->zgvdoktorort).",".
				  " zgvdoktordatum=".$this->addslashes($this->zgvdoktordatum).",".
	...
	{
		/*
		Andreas Koller: Mit dem auskommentierten Qry werden PreInteressenten nicht angezeigt, falls sie schoneinmal preStudent-Status hatten
		...
		*/
		...
		//Andreas Koller: Angepasstes Qry			
		$qry = "SELECT tbl_preinteressent.*, tbl_preinteressentstudiengang.* FROM public.tbl_preinteressent JOIN public.tbl_preinteressentstudiengang USING(preinteressent_id) JOIN public.tbl_person USING(person_id) WHERE 
