...
createLog("ERRORLOG EINTRAEGE VOM: " . $date. "\r\n");
	...
	*/
	...
	{
		echo "SVNR Anmeldung: ";
		echo $svnr_anmeldung = $line['svnr_anmeldung']; 
		echo "<br>";
		...
		"SELECT svnr FROM public.tbl_person WHERE svnr = '$svnr_anmeldung'";
	
		echo $result_svnr_person = pg_query($query_svnr_test_person);
		
		//Eintrag in Logfile bei Fehler
		if (!$result_svnr_person) 
		{
			$date = date('Y-m-d h:i:s');
			createLog($date . " - Abfrage result_svnr_person fehlgeschlagen: " . pg_last_error());
		}
		
		while ($line = pg_fetch_array($result_svnr_person, NULL, PGSQL_ASSOC))
		{
		echo "In der WHILE";
		echo "<br>";
			if (!is_null($line['svnr']))
			{
				echo "Es wurde eine Person zur SVNR gefunden";
				echo "<br>";
		...
		//Kombination
		$query_kombination_person = 
			"SELECT vorname, nachname, gebdatum FROM public.tbl_person WHERE vorname = '$vorname' AND nachname = '$zuname' AND gebdatum = '$gebdatum'";
	
		$result_kombination_person = pg_query($query_kombination_person);
		
		//Eintrag in Logfile bei Fehler
		if (!$result_kombination_person) 
		{
			$date = date('Y-m-d h:i:s');
			createLog($date . " - Abfrage result_kombination_person fehlgeschlagen: " . pg_last_error());
		}
		
		while ($line = pg_fetch_array($result_kombination_person, NULL, PGSQL_ASSOC))
		{
	...
	while ($line = pg_fetch_array($result_felder_test_anmedlung, NULL, PGSQL_ASSOC))
	{
		...
		echo $kombination = $line['vorname'].$line['zuname'].$line['geburtsdatum'];
		...
		else
		{
			...
			$result_insert_person = pg_query($query_insert_person);
			
			//Eintrag in Logfile bei Fehler
			if (!$result_insert_person) 
			{
				$date = date('Y-m-d h:i:s');
				createLog($date . " - Abfrage result_insert_person fehlgeschlagen: " . pg_last_error());
			}
	...
	while ($line = pg_fetch_array($result_felder_test_anmedlung, NULL, PGSQL_ASSOC))
	{
		...
		else
		{
			...
			if ($hauptwohnsitz_ungleich_zustelladresse == 't')
			{
				...
				$result_insert_adresse_1 = pg_query($query_insert_adresse_1);
			
				//Eintrag in Logfile bei Fehler
				if (!$result_insert_adresse_1) 
				{
					$date = date('Y-m-d h:i:s');
					createLog($date . " - Abfrage result_insert_adresse_1 fehlgeschlagen: " . pg_last_error());
				}
			
				$query_insert_adresse_2 =
					"INSERT INTO public.tbl_adresse
					(person_id, strasse, plz, ort, gemeinde, nation, typ, heimatadresse, zustelladresse, insertvon)
					VALUES 
				...
				$result_insert_adresse_2 = pg_query($query_insert_adresse_2);
				
				//Eintrag in Logfile bei Fehler
				if (!$result_insert_adresse_2) 
				{
					$date = date('Y-m-d h:i:s');
					createLog($date . " - Abfrage result_insert_adresse_2 fehlgeschlagen: " . pg_last_error());
				}
			...
			else if ($hauptwohnsitz_ungleich_zustelladresse == 'f')
			{
				...
				$result_insert_adresse_0 = pg_query($query_insert_adresse_0);
				
				//Eintrag in Logfile bei Fehler
				if (!$result_insert_adresse_0) 
				{
					$date = date('Y-m-d h:i:s');
					createLog($date . " - Abfrage result_insert_adresse_0 fehlgeschlagen: " . pg_last_error());
				}
			}
	...
	{	
		...
		else
		{
			...
			$query_insert_kontakt_email =
				"INSERT INTO public.tbl_kontakt
				(person_id, kontakttyp, kontakt, zustellung, insertvon)
				VALUES 
				('$person_id', '$kontakttyp_email', '$kontakt_email', '$zustellung', '$insertvon')
				";

			$result_insert_kontakt_email = pg_query($query_insert_kontakt_email);
			
			//Eintrag in Logfile bei Fehler
			if (!$result_insert_kontakt_email) 
			{
				$date = date('Y-m-d h:i:s');
				createLog($date . " - Abfrage result_insert_kontakt_email fehlgeschlagen: " . pg_last_error());
			}
			
			
			
			#Query für den Telefon-Kontakt
			$query_insert_kontakt_telefon =
				"INSERT INTO public.tbl_kontakt
				(person_id, kontakttyp, kontakt, zustellung, insertvon)
				VALUES 
			...
			$result_insert_kontakt_telefon = pg_query($query_insert_kontakt_telefon);
			
			//Eintrag in Logfile bei Fehler
			if (!$result_insert_kontakt_telefon) 
			{
				$date = date('Y-m-d h:i:s');
				createLog($date . " - Abfrage result_insert_kontakt_telefon fehlgeschlagen: " . pg_last_error());
			}
		}		
	}
	...
	{	
		...
		{
			$query_insert_preinteressent =
				"INSERT INTO public.tbl_preinteressent
				(person_id, studiensemester_kurzbz, aufmerksamdurch_kurzbz, erfassungsdatum, einverstaendnis, anmerkung, insertamum, insertvon, zgv_code, zgvort, anmeldung_id)
				VALUES 
				('$person_id', '$studiensemester_kurzbz', '$aufmerksamdurch_kurzbz', '$erfassungsdatum', '$einverstaendnis', '$anmerkung', '$insertamum', '$insertvon', '$zgv_code', '$zgvort', '$anmeldung_id')";
			
			$result_insert_preinteressent = pg_query($query_insert_preinteressent);
			
			//Eintrag in Logfile bei Fehler
			if (!$result_insert_preinteressent) 
			{
				$date = date('Y-m-d h:i:s');
				createLog($date . " - Abfrage result_insert_preinteressent fehlgeschlagen: " . pg_last_error());
			}	
		}
		...
		else if ($zgvmas_code != NULL && $zgvmas_code != "" && $zgvmas_code != " ")
		{
			$query_insert_preinteressent =
				"INSERT INTO public.tbl_preinteressent
				(person_id, studiensemester_kurzbz, aufmerksamdurch_kurzbz, erfassungsdatum, einverstaendnis, anmerkung, insertamum, insertvon, zgvmas_code, zgvmaort, anmeldung_id)
				VALUES 
				('$person_id', '$studiensemester_kurzbz', '$aufmerksamdurch_kurzbz', '$erfassungsdatum', '$einverstaendnis', '$anmerkung', '$insertamum', '$insertvon', '$zgvmas_code', '$zgvmaort', '$anmeldung_id')";
				
			$result_insert_preinteressent_master = pg_query($query_insert_preinteressent);
			
			//Eintrag in Logfile bei Fehler
			if (!$result_insert_preinteressent_master) 
			{
				$date = date('Y-m-d h:i:s');
				createLog($date . " - Abfrage result_insert_preinteressent_master fehlgeschlagen: " . pg_last_error());
			}	
		...
		else if ($zgvdoktor_code != NULL && $zgvdoktor_code != "" && $zgvdoktor_code != " ")
		{
			$query_insert_preinteressent =
				"INSERT INTO public.tbl_preinteressent
				(person_id, studiensemester_kurzbz, aufmerksamdurch_kurzbz, erfassungsdatum, einverstaendnis, anmerkung, insertamum, insertvon, zgvdoktor_code, zgvdoktorort, anmeldung_id)
				VALUES 
				('$person_id', '$studiensemester_kurzbz', '$aufmerksamdurch_kurzbz', '$erfassungsdatum', '$einverstaendnis', '$anmerkung', '$insertamum', '$insertvon', '$zgvdoktor_code', '$zgvdoktorort', '$anmeldung_id')";
				
			$result_insert_preinteressent_doktor = pg_query($query_insert_preinteressent);
			
			//Eintrag in Logfile bei Fehler
			if (!$result_insert_preinteressent_doktor) 
			{
				$date = date('Y-m-d h:i:s');
				createLog($date . " - Abfrage result_insert_preinteressent_doktor fehlgeschlagen: " . pg_last_error());
			}
		}
	}
	# --- ENDE PREINTERESSENT --- #
	...
	{	
		...
		$query_find_preinteressent_id = 
			"SELECT preinteressent_id
				FROM public.tbl_preinteressent
				WHERE anmeldung_id = '$anmeldung_id'";
		...
		$query_insert_preinteressentstudiengang =
				"INSERT INTO public.tbl_preinteressentstudiengang
				(studiengang_kz, preinteressent_id, prioritaet, freigabedatum, insertamum, insertvon, updatevon, orgform_kurzbz)
				VALUES 
				('$studiengang_1', '$preinteressent_id', $prioritaet, '$freigabedatum', '$insertamum', '$insertvon', '$updatevon', '$orgform_kurzbz_1')";
					
		$preintstudiengang = pg_query($query_insert_preinteressentstudiengang);
		...
		if (!is_null($studiengang_2))
		{
			$query_insert_preinteressentstudiengang =
				...
				(studiengang_kz, preinteressent_id, prioritaet, freigabedatum, insertamum, insertvon, updatevon, orgform_kurzbz)
				VALUES 
				('$studiengang_2', '$preinteressent_id', $prioritaet, '$freigabedatum', '$insertamum', '$insertvon', '$updatevon', '$orgform_kurzbz_2')";
		...
		if (!is_null($studiengang_3))
		{
			$query_insert_preinteressentstudiengang =
				...
				(studiengang_kz, preinteressent_id, prioritaet, freigabedatum, insertamum, insertvon, updatevon, orgform_kurzbz)
				VALUES 
				('$studiengang_3', '$preinteressent_id', $prioritaet, '$freigabedatum', '$insertamum', '$insertvon', '$updatevon', '$orgform_kurzbz_3')";
		...
		if (!is_null($studiengang_4))
		{
			$query_insert_preinteressentstudiengang =
				...
				(studiengang_kz, preinteressent_id, prioritaet, freigabedatum, insertamum, insertvon, updatevon, orgform_kurzbz)
				VALUES 
				('$studiengang_4', '$preinteressent_id', $prioritaet, '$freigabedatum', '$insertamum', '$insertvon', '$updatevon', '$orgform_kurzbz_4')";
