...
else if ($_GET["seite"] == "leaufteilung")
{
	...
	*/
	...
	$query_alle = 
	"SELECT DISTINCT studiengang_kz, mitarbeiter_uid, stunde, datum, semester
	FROM lehre.tbl_stundenplan
	WHERE datum BETWEEN '2014-09-01' AND '2015-02-19'
	ORDER BY datum, stunde, studiengang_kz ASC";
	
	$query_intern = 
	"SELECT DISTINCT sP.studiengang_kz, sP.mitarbeiter_uid, sP.stunde, sP.datum, sP.semester, mA.fixangestellt
	...
	$query_intern_studiengangsleiter = 
	"SELECT DISTINCT sP.studiengang_kz, sP.mitarbeiter_uid, sP.stunde, sP.datum, sP.semester, mA.fixangestellt
	FROM lehre.tbl_stundenplan AS sP
	INNER JOIN public.tbl_mitarbeiter AS mA
	ON sP.mitarbeiter_uid = mA.mitarbeiter_uid
	INNER JOIN public.tbl_benutzerfunktion AS bF
	ON bF.uid = mA.mitarbeiter_uid
	WHERE datum BETWEEN '2014-09-01' AND '2015-02-19'
	AND fixangestellt = 't'
	...
	{	
		...
		{
			array_push($studiengang_array, $line['studiengang_kz']);
		}
	}
		
	echo "<table border = 1><tr><td>Studiengang/Department</td><td>LE Intern</td><td>%</td><td>(LE Intern Studiengangsleiter)</td><td>LE Extern</td><td>%</td><td>LE Summe</td><td>%</td></tr>";

	# SCHLEIFE HIER BEGINNEN
	foreach ($studiengang_array as &$studiengang)
	{
		$result_alle = pg_query($query_alle) or die('Abfrage fehlgeschlagen: ' . pg_last_error());
	
		$result_intern = pg_query($query_intern) or die('Abfrage fehlgeschlagen: ' . pg_last_error());
		
		$result_intern_studiengangsleiter = pg_query($query_intern_studiengangsleiter) or die('Abfrage fehlgeschlagen: ' . pg_last_error());
	
		$result_extern = pg_query($query_extern) or die('Abfrage fehlgeschlagen: ' . pg_last_error());

		$counter_alle = 1;
		$counter_intern = 1;
		$counter_intern_studiengangsleiter = 1;
		$counter_extern = 1;
		
		while ($line = pg_fetch_array($result_alle, NULL, PGSQL_ASSOC)) 
		{	
		...
		while ($line = pg_fetch_array($result_intern_studiengangsleiter, NULL, PGSQL_ASSOC))
		{
			if ($line['studiengang_kz'] == $studiengang)
			{
				$counter_intern_studiengangsleiter++;
			}
		}
		...
		$offiziell_intern = round(($counter_alle*($prozent_intern / 100)),0);
		$offiziell_extern = round(($counter_alle*($prozent_extern / 100)),0);
		
		echo "<tr><td>$studiengang</td><td>$offiziell_intern</td><td>$prozent_intern</td><td>$counter_intern_studiengangsleiter</td><td>$offiziell_extern</td><td>$prozent_extern</td><td>$counter_alle</td><td>100</td></tr>";
		
		/*
		echo "<br>";
		echo $studiengang;
		echo " ";
		echo $mmig_counter_alle;
		echo " Intern: ";
		echo $mmig_counter_intern;
		echo " Extern: ";
		echo $mmig_counter_extern;
		echo " Intern Prozent: ";
		echo $mmig_prozent_intern;
		echo " Extern Prozent: ";
		echo $mmig_prozent_extern;
		echo " Intern Offiziell: ";
		echo $mmig_offiziell_intern;
		echo " Extern Offiziell: ";
		echo $mmig_offiziell_extern;
		echo "<br>";
		*/
	}
	...
	foreach ($studiengaenge as &$studiengang)
	{
		...
		$query_anzahlbeweber = 
			"SELECT COUNT(status_kurzbz)
				...
				WHERE pStatus.status_kurzbz = 'Bewerber' 
...
else if ($_GET["seite"] == "swsjestudiengang")
{
	...
	echo "<table border = 1><tr><td>Studiengang</td><td>SWS Studiengangsleitung</td><td>SWS Intern</td><td>SWS Extern</td></tr>";

	# SCHLEIFE ÜBER ALLE STUDIENGÄNGE
	foreach ($studiengaenge as &$studiengang)
	{
		$query_SWS_intern_lektor = 
			"SELECT SUM(semesterstunden)
				FROM lehre.vw_lva_stundenplan AS sP
				INNER JOIN public.tbl_mitarbeiter AS mA
				ON sP.lektor_uid = mA.mitarbeiter_uid
				WHERE studiengang_kz = $studiengang
				AND studiensemester_kurzbz = 'WS2014'
				AND mA.fixangestellt = 'true'
				AND semesterstunden IS NOT NULL
				AND semesterstunden != '0'";
				
		$query_SWS_extern_lektor = 
			"SELECT SUM(semesterstunden)
				FROM lehre.vw_lva_stundenplan AS sP
				INNER JOIN public.tbl_mitarbeiter AS mA
				ON sP.lektor_uid = mA.mitarbeiter_uid
		...
		$query_SWS_intern_leitung = 
			"SELECT SUM(semesterstunden)
				FROM lehre.vw_lva_stundenplan AS sP
				INNER JOIN public.tbl_mitarbeiter AS mA
				ON sP.lektor_uid = mA.mitarbeiter_uid
				INNER JOIN public.tbl_benutzerfunktion AS bF
				ON mA.mitarbeiter_uid = bF.uid
				WHERE studiengang_kz = $studiengang
				AND studiensemester_kurzbz = 'WS2014'
				AND mA.fixangestellt = 'true'
				AND semesterstunden IS NOT NULL
		...
		#SWS Intern Lektor
		$result_SWS_intern_lektor = pg_query($query_SWS_intern_lektor) or die('Abfrage fehlgeschlagen: ' . pg_last_error());
		
		while ($line = pg_fetch_array($result_SWS_intern_lektor, NULL, PGSQL_ASSOC)) 
		{	
			$sws_intern = $line['sum'];
		}
			
		#SWS Extern Lektor
		$result_SWS_extern_lektor = pg_query($query_SWS_extern_lektor) or die('Abfrage fehlgeschlagen: ' . pg_last_error());
		
		while ($line = pg_fetch_array($result_SWS_extern_lektor, NULL, PGSQL_ASSOC)) 
		{	
			$sws_extern = $line['sum'];
		}
		
		#SWS Leitung Lektor
		$result_SWS_intern_leitung = pg_query($query_SWS_intern_leitung) or die('Abfrage fehlgeschlagen: ' . pg_last_error());
		
		while ($line = pg_fetch_array($result_SWS_intern_leitung, NULL, PGSQL_ASSOC)) 
		{	
			$sws_leitung = $line['sum'];
		}
		
		echo "<tr><td>$studiengang</td><td>$sws_leitung</td><td>$sws_intern</td><td>$sws_extern</td></tr>";
