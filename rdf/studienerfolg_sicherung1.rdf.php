...
function draw_studienerfolg($uid, $studiensemester_kurzbz)
{
	...
	$query = "SELECT 
				...
				tbl_person.vornamen, tbl_person.vorname, tbl_person.nachname,tbl_person.gebdatum, 
	...
	$xml .= "		<vornamen>".$row->vornamen."</vornamen>";
	...
	foreach ($obj->result as $row)
	{
		if($row->zeugnis)
		{
			...
			//Beginn Änderungen von Andreas Koller:
			//LVs bzw. Module welche mit "Lehre = FALSE" gekennzeichnet sind und 
			//Lehrveranstaltungen die negativ benotet wurden, werden nicht in die Erfolgsbestätigung miteinbezogen
			...
			$qry_lehre = "SELECT lehre
							FROM lehre.tbl_lehrveranstaltung
							WHERE lehrveranstaltung_id = '$lehrveranstaltung_id'";
			
			$result_lehre = pg_query($qry_lehre);
			
			pg_close($dbconn);
			
			$lehre = NULL;
							
			while ($line = pg_fetch_array($result_lehre, NULL, PGSQL_ASSOC))
			{
				 $lehre = $line['lehre']; 
			}
			
			if($note!='' && $note!=5 && $lehre=='t')
			//Ende Änderungen von Andreas Koller
			{
				$xml .= "			<unterrichtsfach>";
