...
$db = new basis_db();
	...
	{	
		...
		//Andreas Koller - Neue Variablen eingefügt:
		$ebenencounter = 1;
		$notezweiterebene = NULL;
		$masterarbeit = 'f';
...
{
	global $ects_gesamt, $ects_absolviert,$studienplan,$note_arr,$ebenencounter,$notezweiterebene;
	$xml='';
	$row = $obj['data'];
	if($row->zeugnis)
	{
		...
		//Andreas Koller - Wenn Note "an", dann soll Note den Wert "m.E.tg" erhalten
		if ($row->note == 6)
		{
			$note = "m.E.tg.";
		}
		
		//TODO: Wenn in der 2. Ebene eine Note eingetragen wurde, DANN soll in der 3. Ebene nur ein Strich bei der Note stehen
		
		//Note der 2. Ebene in eigene Variable speichern
		if ($ebenencounter == 2)
		{
			$notezweiterebene = $note;
		}
		
		if (($ebenencounter == 3) && ($notezweiterebene != NULL))
		{
			//$ectspunkte = "-";
			//$sws = "-";
			$note = "-";
		...
		//TODO: Prüfung einbauen - Wenn es sich bei der aktuellen LV ($bezeichnung) um die Masterthesis handelt, so soll ein * neben der LV Bezeichnung sein UND in der Fußnote
		//	    folgender Hinweis stehen:
		//		"Beurteilung der Masterarbeit im Rahmen der kommissionellen Prüfung gemäß FHStG §16 (2)"
		//		Hierfür sollte an dieser Stelle auch ein FLAG für die Fußnote gesetzt werden.
		
		if ($bezeichnung == "Masterarbeit" OR $bezeichnung == "Masterarbeit und kommissionelle Prüfung" OR $bezeichnung == "Master's Thesis")
		{
			$bezeichnung = $bezeichnung."*";
			$masterarbeit = 't';
			//Wortlauf für Fußnote: Beurteilung der Masterarbeit im Rahmen der kommissionellen Prüfung gemäß FHStG §16 (2)
		}
		...
		//Zusätzliche Prüfung - Wenn Note leer, dann sollen auch keine ECTS angezeigt werden
		if($row->ects==0 || $row->ects=='' || $note==NULL)
			$ectspunkte='';
		else
		{
		...
		//Zusätzliche Prüfung eingebaut -> Wenn ECTS bereits mit dem Modul zu ects_absolviert addiert wurden,
		//dann werden sie nicht noch einmal addiert.
		if (($ebenencounter == 3) && ($notezweiterebene != NULL))
		{
			$ectspunkte = "-";
			//$ects_absolviert soll nicht erweitert werden, da die ECTS bereits von der Modulnote addiert wurden
		}
		else if($note_arr[$row->note]['positiv'])
		{
