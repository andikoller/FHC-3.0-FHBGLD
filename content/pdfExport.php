	...
	foreach ($uids as $uid)
	{
		...
		if($result_ma = $db->db_query($qry))
		{
			if($db->db_num_rows($result_ma)==1)
			{
				...
				//Andreas Koller: Adminrechte erforderlich
		...
		if($result_std = $db->db_query($qry))
		{
			if($db->db_num_rows($result_std)==1)
			{
				...
				//Rechte pruefen
				if ($rechte->isBerechtigt('admin', $row_std->studiengang_kz, 'suid') || $rechte->isBerechtigt('admin', 0, 'suid')) 
				   //||		   
...
'PrProtBAEng','PrProtMAEng','Studienordnung','Erfolgsnachweis','ErfolgsnwHead','Studienblatt','LV_Informationen','ErfolgsnachweisE','ErfolgsnwHeadE')))
{
