...
$stgobj->load($stg);
//Logo
$pdf->Image("../../../skin/images/logo.jpg","447","45","","60","jpg","");
...
$maxX=30;
$pdf->SetXY($maxX,$maxY);
$pdf->MultiCell(280,$lineheight,'Anzahl der abgehaltenen Unterrichtseinheiten',1,'L',0);
$maxX +=280;
$pdf->SetXY($maxX,$maxY);
$pdf->MultiCell(40,$lineheight,'',1,'L',0);
$maxX +=40;
...
if($result = $db->db_query($qry))
{
	while($row=$db->db_fetch_object($result))
	{
		...
		$vorname=mb_convert_encoding(trim(replaceUTF8ProblemCharacters($row->vorname)),'ISO-8859-15','UTF-8');
		$nachname=mb_convert_encoding(trim(replaceUTF8ProblemCharacters($row->nachname)),'ISO-8859-15','UTF-8');
...
$pdf->SetFont('Arial','',8);
$pdf->SetXY(30,$maxY);
$inhalt[]=array(' ',mb_convert_encoding('Hörer/Name','ISO-8859-15','UTF-8'),'Kennzeichen','Gruppe','','','','','',''); //Spaltenueberschriften
...
function replaceUTF8ProblemCharacters($str)
{
...
if($result = $db->db_query($qry))
{
	...
	while($elem = $db->db_fetch_object($result))
	{
		...
		if($elem->status!='Abbrecher' && $elem->status!='Unterbrecher')
		{
			...
			$vorname=mb_convert_encoding(trim(replaceUTF8ProblemCharacters($elem->vorname)),'ISO-8859-15','UTF-8');
			$nachname=mb_convert_encoding(trim(replaceUTF8ProblemCharacters($elem->nachname)),'ISO-8859-15','UTF-8');
			$name="$vorname $nachname";
...
$pdf->SetXY($maxX,$maxY);
$pdf->SetFont('Arial','',8);
$pdf->MultiCell(520,$lineheight,mb_convert_encoding('Für Studierende gilt die in der Studienordnung vorgegebene Anwesenheitsvorgabe, bitte um einen deutlichen Vermerk auf der Anwesenheitsliste. Die Anwesenheitsliste bitte am Ende der Lehrveranstaltung im Sekretariat abgeben! Bitte achten Sie darauf, dass Sie nur VOLLSTÄNDIG AUSGEFÜLLTE LISTEN abgeben!','ISO-8859-15','UTF-8'),0,'L',0);
